...
void RTC_Set_Calendar_Date(uint32_t WeekDay, uint32_t Day, uint32_t Month, uint32_t Year) {
	...
	// set registers
	RTC->DR ^= (RTC->DR ^ (WeekDay<<RTC_POSITION_DR_WDU | __RTC_CONVERT_BIN2BCD(Day)<<RTC_POSITION_DR_DU | __RTC_CONVERT_BIN2BCD(Year)<<RTC_POSITION_DR_YU | Month<<RTC_POSITION_DR_MU)) & (7UL<<RTC_POSITION_DR_WDU | 63UL<<RTC_POSITION_DR_DU | 31UL<<RTC_POSITION_DR_MU | 255UL<<RTC_POSITION_DR_YU);
}

void RTC_Set_Time(uint32_t Format12_24, uint32_t Hour, uint32_t Minute, uint32_t Second) {
	RTC->TR ^= (RTC->TR ^ (Format12_24 | __RTC_CONVERT_BIN2BCD(Hour)<<RTC_POSITION_TR_HU | __RTC_CONVERT_BIN2BCD(Minute)<<RTC_POSITION_TR_MU | __RTC_CONVERT_BIN2BCD(Second)<<RTC_POSITION_TR_SU)) & (1UL<<(RTC_POSITION_TR_HT + 2) | 63UL<<RTC_POSITION_TR_HU | 127UL<<RTC_POSITION_TR_MU | 127UL<<RTC_POSITION_TR_SU);
}
...
uint32_t RTC_TIME_GetHour(void) {
	return (((63UL << RTC_POSITION_TR_HU) & RTC->TR)>>RTC_POSITION_TR_HU);
}

uint32_t RTC_TIME_GetMinute(void) {
	return (((127UL << RTC_POSITION_TR_MU) & RTC->TR)>>RTC_POSITION_TR_MU);
}

uint32_t RTC_TIME_GetSecond(void) {
	return (((127UL << RTC_POSITION_TR_SU) & RTC->TR)>>RTC_POSITION_TR_SU);
}

uint32_t RTC_DATE_GetMonth(void) {
	return ((31UL<<RTC_POSITION_DR_MU & RTC->DR)>>RTC_POSITION_DR_MU);
}

uint32_t RTC_DATE_GetDay(void) {
	return ((63UL<<RTC_POSITION_DR_DU & RTC->DR)>>RTC_POSITION_DR_DU);
}

uint32_t RTC_DATE_GetYear(void) {
	return ((255UL<<RTC_POSITION_DR_YU & RTC->DR)>>RTC_POSITION_DR_YU);
}

uint32_t RTC_DATE_GetWeekDay(void) {
	return ((7UL<<RTC_POSITION_DR_WDU & RTC->DR)>>RTC_POSITION_DR_WDU);
}
